...
use App\Console\Commands\PlanColdStart;
...
class GlobalCommandReferenceTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        parent::setUp();
        $reference = dirname(__DIR__);
        $this->app->register(GlobalCommandServiceProvider::class);
        $this->app->make(Kernel::class)->registerCommand(new PlanColdStart);
        foreach (glob($reference.'/database/migrations/*.php') as $file) {
            (require $file)->up();
        }
        config(['global-command' => require $reference.'/config/global-command.php']);
        config(['global-command.enabled' => true]);
        app('view')->addLocation($reference.'/resources/views');
        Blade::anonymousComponentPath($reference.'/resources/views/components');
        Livewire::component('admin.geo-switcher', GeoSwitcher::class);
        Livewire::component('reference-liquidity', CountryLiquidityWidget::class);
        Livewire::component('reference-radar', DiasporaMediaRadar::class);
        Route::get('/__reference/global', fn () => 'reference')->name('filament.admin.pages.global-command-center');
        Route::get('/__reference/radar', fn () => 'reference')->name('filament.admin.pages.diaspora-medya-radari');
    ...
    public function test_switcher_saves_only_validated_context_and_rejects_non_admin(): void
    {
        Livewire::actingAs($this->admin())->test(GeoSwitcher::class)
            ->set('mode', 'country')->set('countryCode', 'FJ')->call('apply')->assertHasNoErrors()->assertRedirect('/__reference/global');
