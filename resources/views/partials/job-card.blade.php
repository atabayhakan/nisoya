<a href="{{ route('jobs.show', [$job, $job->slug]) }}"
   class="group block rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-stone-900 dark:shadow-none {{ $job->isCurrentlyFeatured() ? 'border-amber-300 ring-1 ring-amber-200 dark:border-amber-700 dark:ring-amber-900/40' : 'border-stone-200 dark:border-stone-800' }}">
    <div class="flex items-start gap-4">
        {{-- Şirket logosu --}}
        <div class="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl bg-stone-100 font-bold text-stone-500 dark:bg-stone-800 dark:text-stone-400">
        ...
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-1.5 text-xs">
                ...
                @endif
                @if ($job->category)
                    <span class="rounded-full bg-stone-100 px-2 py-0.5 text-stone-500 dark:bg-stone-800 dark:text-stone-400">{{ $job->category->name }}</span>
                @endif
            ...
            <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-stone-400 dark:text-stone-500">
                @if ($job->country)
