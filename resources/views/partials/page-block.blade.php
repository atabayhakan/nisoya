...
@switch($block['type'] ?? '')
    @case('baslik')
        ...
        <{{ $lvl }} class="{{ $lvl === 'h2' ? 'text-2xl' : 'text-xl' }} font-bold text-stone-900">{{ $data['text'] ?? '' }}</{{ $lvl }}>
        @break
    ...
    @case('gorsel')
        <figure>
            ...
            @if (! empty($data['caption']))
                <figcaption class="mt-2 text-center text-sm text-stone-500">{{ $data['caption'] }}</figcaption>
            @endif
        </figure>
        @break
