    ...
    <div class="mx-auto max-w-6xl px-4 pb-10 pt-5">
        ...
        <div class="mt-4 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <div class="grid gap-6">
                ...
                @if ($listing->images->isNotEmpty())
                    ...
                    <div class="grid gap-3 {{ $yanGorseller->isNotEmpty() ? 'sm:grid-cols-[minmax(0,1fr)_176px]' : '' }}">
                        <div class="relative overflow-hidden rounded-3xl border border-stone-200/80 bg-stone-900/5 shadow-2xs dark:border-stone-800 dark:bg-stone-900/60">
                            {{-- Klasik şablonla AYNI sözleşme: görsel kırpılmaz,
                                 zemini kendi bulanık kopyası doldurur. --}}
                            <img src="{{ $heroMedium }}" alt="" aria-hidden="true" loading="lazy"
                                 class="pointer-events-none absolute inset-0 h-full w-full scale-110 object-cover blur-2xl saturate-150 opacity-40 dark:opacity-20">
                            <img src="{{ $heroLarge }}"
                                 ...
                                 class="listing-cover-transition relative mx-auto h-[280px] w-full object-contain sm:h-[380px]">
                            @if ($listing->images->count() > 1)
                                <span class="absolute bottom-3.5 left-3.5 rounded-full bg-stone-900/80 px-3 py-1 text-xs font-bold text-white shadow-xs backdrop-blur-xs dark:bg-stone-950/80">
                                    1 / {{ $listing->images->count() }}
                                </span>
                            @endif
                            <x-temsili-rozet :gorsel="$hero" buyuk class="absolute bottom-3.5 right-3.5" />
                        </div>
                        ...
                        @if ($yanGorseller->isNotEmpty())
                            <div class="grid grid-cols-3 gap-3 sm:grid-cols-1 sm:grid-rows-3">
                                @foreach ($yanGorseller as $image)
                                    ...
                                    <div class="relative overflow-hidden rounded-2xl border border-stone-200/70 bg-stone-100 dark:border-stone-800 dark:bg-stone-800">
                                        <img src="{{ $thumb }}"
                                             ...
                                             class="h-full min-h-[76px] w-full object-cover sm:h-[115px] transition duration-200 hover:scale-105">
                                        @if ($loop->last && $kalanFoto > 0)
                                            <span class="absolute inset-0 grid place-items-center bg-stone-900/40 backdrop-blur-xs">
                                                <span class="rounded-xl bg-white/90 px-3 py-1.5 text-xs font-bold text-stone-800 shadow-xs dark:bg-stone-900/90 dark:text-stone-100">+{{ $kalanFoto }} foto</span>
                                            </span>
                                        @endif
                                    </div>
                                @endforeach
                ...
                @else
                    ...
                    <div style="--listing-transition-name: listing-image-{{ $listing->id }}" class="listing-cover-transition flex h-64 items-center justify-center rounded-3xl border border-stone-200/80 bg-stone-50 text-stone-300 dark:border-stone-800 dark:bg-stone-800 dark:text-stone-400">
                ...
                <div class="rounded-3xl border border-stone-200/90 bg-white p-6 sm:p-8 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                    ...
                    <h1 class="mt-4 text-2xl sm:text-3xl font-extrabold tracking-tight text-stone-900 dark:text-stone-100" style="text-wrap: pretty">{{ $listing->title }}</h1>

                    <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-stone-100 pb-4 dark:border-stone-800">
                        <span class="text-2xl sm:text-3xl font-extrabold text-emerald-700 dark:text-emerald-400">
                        ...
                        @endif
                        <span class="hidden h-5 w-px bg-stone-200 sm:block dark:bg-stone-700" aria-hidden="true"></span>
                    ...
                    <div class="prose prose-stone mt-5 max-w-none text-base font-normal leading-relaxed text-stone-600 dark:prose-invert dark:text-stone-300">
                ...
                @if ($listing->type->value === 'emlak' && $listing->propertyDetail)
                    ...
                    <div class="rounded-3xl border border-stone-200/90 bg-white p-6 sm:p-8 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                        <h2 class="text-base sm:text-lg font-bold text-stone-900 dark:text-stone-100">🏡 Emlak Özellikleri</h2>
                        <dl class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                            ...
                            @foreach ($emlakAlanlari as [$etiket, $deger])
                                <div class="rounded-2xl border border-stone-100 bg-stone-50/70 p-3.5 dark:border-stone-800 dark:bg-stone-800/40">
                                    <dt class="text-xs font-semibold text-stone-500 dark:text-stone-400">{{ $etiket }}</dt>
                                    <dd class="mt-1 text-sm font-bold text-stone-900 dark:text-stone-100">{{ $deger }}</dd>
                                </div>
                            @endforeach
                        </dl>
                        @if ($detail->badgeLabels())
                            <div class="mt-5 flex flex-wrap gap-2 border-t border-stone-100 pt-4 dark:border-stone-800">
                                @foreach ($detail->badgeLabels() as $badge)
                                    <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-600/20 dark:bg-emerald-950/60 dark:text-emerald-300">✓ {{ $badge }}</span>
                                @endforeach
                            </div>
                        @endif
                    </div>
                ...
                @if ($listing->type->value === 'vasita' && $listing->vehicleDetail)
                    ...
                    <div class="rounded-3xl border border-stone-200/90 bg-white p-6 sm:p-8 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                        <h2 class="text-base sm:text-lg font-bold text-stone-900 dark:text-stone-100">🚗 Araç Özellikleri</h2>
                        <dl class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                            @php
                ...
                {{-- Değerlendirmeler (Faz P4) --}}
                @if ($recentReviews->isNotEmpty())
                    <div class="rounded-3xl border border-stone-200/90 bg-white p-6 sm:p-8 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <h2 class="text-base sm:text-lg font-bold text-stone-900 dark:text-stone-100">Değerlendirmeler</h2>
                            @if ($sellerRating['count'] > 0)
                                <div class="flex items-baseline gap-2">
                                    <span class="text-base sm:text-lg font-extrabold text-stone-900 dark:text-stone-100">★ {{ $sellerRating['avg'] }}</span>
                        ...
                        <div class="mt-4 grid gap-3">
                            @foreach ($recentReviews as $review)
                                <div class="flex gap-3 rounded-2xl border border-stone-100 bg-stone-50/70 p-4 dark:border-stone-800 dark:bg-stone-800/40">
                                    <x-avatar :user="$review->reviewer" size="h-10 w-10" text="text-sm" />
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-baseline gap-2">
                                            <span class="text-sm font-bold text-stone-900 dark:text-stone-100">{{ $review->reviewer->name }}</span>
                        ...
                        @if ($sellerRating['count'] > $recentReviews->count())
                            <a href="{{ route('profiles.show', $listing->user->username) }}" class="mt-3 inline-block text-xs font-bold text-emerald-700 hover:underline dark:text-emerald-400">
                ...
                @auth
                    @unless ($isOwner)
                        <details class="text-sm">
                            <summary class="inline-flex cursor-pointer items-center gap-1 font-semibold text-stone-600 hover:text-stone-800 dark:text-stone-400 dark:hover:text-stone-300">
                                <x-heroicon-o-flag class="h-4 w-4" /> Bu ilanı şikayet et
                            </summary>
                            <form method="POST" action="{{ route('reports.store', $listing) }}" class="mt-3 max-w-md space-y-3 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                                @csrf
                                <select name="reason" required class="w-full rounded-xl border-stone-200 px-3 py-2 text-xs font-semibold text-stone-800 focus:border-emerald-500 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100">
                                ...
                                <textarea name="note" rows="2" placeholder="Eklemek istediğin not (ops.)" class="w-full rounded-xl border-stone-200 px-3 py-2 text-xs text-stone-800 placeholder-stone-400 focus:border-emerald-500 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100 dark:placeholder-stone-500"></textarea>
                                <button type="submit" class="rounded-xl bg-stone-900 px-4 py-2 text-xs font-bold text-white transition hover:bg-stone-800 dark:bg-stone-800 dark:hover:bg-stone-700">Şikayeti gönder</button>
                            </form>
                        </details>
                    @endunless
                @else
            ...
            <div class="grid gap-4 lg:sticky lg:top-[90px] lg:self-start">
                ...
                {{-- Fiyat + aksiyon kartı --}}
                <div id="satici-iletisim" class="scroll-mt-24 rounded-3xl border border-stone-200/90 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                    <div class="flex items-baseline justify-between gap-2 border-b border-stone-100 pb-4 dark:border-stone-800">
                        <span class="text-3xl font-extrabold text-stone-900 dark:text-stone-50">
                            @if ($listing->price !== null)
                                {{ $listing->bicimliFiyat() }} {{ $listing->currency }}
                            @else
                    ...
                    <div class="mt-5 space-y-3">
                        @if ($isArchived && ! $isOwner)
                            <div class="rounded-2xl bg-stone-100 px-4 py-3.5 text-center text-xs font-bold text-stone-600 dark:bg-stone-800 dark:text-stone-300">
                                Bu ilan yayından kalktığı için mesaj gönderilemez.
                            </div>
                        @elseif ($isOwner)
                            <a href="{{ route('panel.listings.edit', $listing) }}" class="inline-flex items-center justify-center gap-2 w-full rounded-xl bg-emerald-700 px-4 py-3 text-center text-sm font-bold text-white shadow-sm transition hover:bg-emerald-800">
                        ...
                        @elseif (auth()->check())
                            ...
                            @endphp
                            <form method="POST" action="{{ route('messages.start', $listing) }}" class="space-y-3">
                                @csrf
                                @if ($isShortTerm)
                                    <div class="rounded-2xl border border-stone-200/80 bg-stone-50 p-3.5 dark:border-stone-700 dark:bg-stone-800/60">
                                        <p class="text-xs font-semibold text-stone-600 dark:text-stone-300">📅 Tarih seç, talep mesajına eklensin <span class="font-normal">(ops.)</span></p>
                                        <div class="mt-2.5 grid grid-cols-2 gap-2">
                                            <input name="giris" type="date" value="{{ old('giris') }}" aria-label="Giriş tarihi"
                                                   class="w-full rounded-xl border-stone-200 bg-white px-2.5 py-2 text-xs font-semibold text-stone-800 focus:border-emerald-500 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100">
                                        ...
                                        <input name="kisi" type="number" min="1" max="50" value="{{ old('kisi') }}" placeholder="Kişi sayısı (ops.)"
                                               class="mt-2 w-full rounded-xl border-stone-200 bg-white px-2.5 py-2 text-xs font-semibold text-stone-800 focus:border-emerald-500 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100 dark:placeholder-stone-500">
                                ...
                                <textarea name="body" rows="3" required placeholder="{{ $listing->type->value === 'emlak' ? 'İlan sahibine bir mesaj yaz...' : 'Satıcıya bir mesaj yaz...' }}"
                                          class="w-full rounded-xl border border-stone-200 bg-stone-50/70 px-3.5 py-2.5 text-xs text-stone-800 placeholder-stone-400 focus:border-emerald-500 focus:bg-white focus:ring-1 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100 dark:placeholder-stone-500">{{ old('body') }}</textarea>
                                @error('body') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                <x-button type="submit" class="h-12 w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl shadow-xs transition">
                                    <x-heroicon-o-chat-bubble-left class="h-4 w-4" /> Mesaj Gönder
                                </x-button>
                            </form>
                        @else
                            <x-button :href="route('login')" class="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl h-12 shadow-xs transition">Mesaj göndermek için giriş yap</x-button>
                        @endif
                        ...
                        @auth
                            @unless ($isOwner)
                                <form method="POST" action="{{ route('favorites.toggle', $listing) }}">
                                    @csrf
                                    <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-bold transition {{ $isFavorited ? 'border-red-200 bg-red-50 text-red-600 dark:border-red-800 dark:bg-red-950/30 dark:text-red-400' : 'border-stone-200 bg-stone-50 text-stone-700 hover:bg-stone-100 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-200 dark:hover:bg-stone-700' }}">
                                        @if ($isFavorited)
                                            <x-heroicon-s-heart class="h-4 w-4" /> Favorilerde
                                        @else
                                            <x-heroicon-o-heart class="h-4 w-4" /> Favorilere ekle
                                        @endif
                                    </button>
                                </form>
                            @endunless
                    ...
                    <div class="mt-5 border-t border-stone-100 pt-4 dark:border-stone-800">
                        <p class="mb-2.5 text-xs font-bold uppercase tracking-wider text-stone-600 dark:text-stone-400">Bu İlanı Paylaş</p>
                ...
                @if (in_array($listing->type->value, ['emlak', 'vasita'], true) && $listing->relationLoaded('unavailableRanges'))
                    <div class="rounded-3xl border border-stone-200/90 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                        <h2 class="text-sm font-bold text-stone-900 dark:text-stone-100">📅 Müsaitlik Takvimi</h2>
                        <div class="mt-3">
                ...
                {{-- SATICI KARTI --}}
                <div class="overflow-hidden rounded-3xl border border-stone-200/90 bg-white shadow-sm dark:border-stone-800 dark:bg-stone-900">
                    <div class="h-1.5 w-full bg-emerald-600 dark:bg-emerald-500" aria-hidden="true"></div>
                    <div class="p-6">
                        <div class="flex items-center gap-3.5">
                            ...
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-1.5 text-base font-bold leading-tight text-stone-900 dark:text-stone-100">
                                    {{ $listing->user->name }}
                        ...
                        <div class="mt-5 grid grid-cols-2 gap-2.5">
                            <div class="rounded-2xl border border-stone-100 bg-stone-50/80 p-3.5 dark:border-stone-800 dark:bg-stone-800/50">
                                <div class="text-[11px] font-semibold uppercase tracking-wide text-stone-600 dark:text-stone-400">Puan</div>
                                <div class="mt-1 text-sm font-bold text-stone-900 dark:text-stone-100">
                                    @if ($sellerRating['count'] > 0)
                                        ★ {{ $sellerRating['avg'] }} <span class="text-xs font-medium text-stone-500 dark:text-stone-400">({{ $sellerRating['count'] }})</span>
                                    @else
                                        <span class="text-xs font-medium text-stone-500 dark:text-stone-400">Henüz yok</span>
                                    @endif
                                </div>
                            </div>
                            <div class="rounded-2xl border border-stone-100 bg-stone-50/80 p-3.5 dark:border-stone-800 dark:bg-stone-800/50">
                                <div class="text-[11px] font-semibold uppercase tracking-wide text-stone-600 dark:text-stone-400">Üyelik</div>
                                <div class="mt-1 text-sm font-bold text-stone-900 dark:text-stone-100">{{ $listing->user->created_at->year }}</div>
                            </div>
                        </div>
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-1.5">
                                @foreach ($listing->user->paymentLinks as $link)
                                    @if ($link->detailIsLink())
                                        <a href="{{ $link->detail }}" target="_blank" rel="noopener nofollow" class="inline-flex items-center gap-1 rounded-xl border border-stone-200/80 bg-stone-50 px-2.5 py-1 text-xs font-semibold text-stone-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:border-emerald-600 dark:hover:text-emerald-300" title="{{ $link->method->getLabel() }} — kendi ödeme sayfasına git">
                                            <span aria-hidden="true">{{ $link->method->icon() }}</span>{{ $link->method->getLabel() }} ↗
                                        </a>
                                    @elseif ($link->qr_path)
                                        <a href="{{ Storage::url($link->qr_path) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 rounded-xl border border-stone-200/80 bg-stone-50 px-2.5 py-1 text-xs font-semibold text-stone-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:border-emerald-600 dark:hover:text-emerald-300" title="{{ $link->method->getLabel() }} — QR kodu gör">
                                            <span aria-hidden="true">{{ $link->method->icon() }}</span>{{ $link->method->getLabel() }} 🔳
                                        </a>
                                    @else
                                        <span class="inline-flex items-center gap-1 rounded-xl border border-stone-200/80 bg-stone-50 px-2.5 py-1 text-xs font-semibold text-stone-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300">
                        ...
                        <a href="{{ route('profiles.show', $listing->user->username) }}" class="mt-3 block text-xs font-bold text-emerald-700 hover:underline dark:text-emerald-400">Profili ve değerlendirmeleri gör →</a>
                ...
                @if ($similarListings->isNotEmpty())
                    <div class="rounded-3xl border border-stone-200/90 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900">
                        <h2 class="text-xs font-bold uppercase tracking-wider text-stone-900 dark:text-stone-100">Benzer İlanlar</h2>
                        <div class="mt-4 grid gap-3">
