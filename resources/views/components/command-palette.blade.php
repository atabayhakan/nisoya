        ...
        >
            ...
            >
                <div class="flex items-center gap-3 border-b border-stone-100 px-4 py-3 dark:border-stone-800">
                    <x-heroicon-o-magnifying-glass class="h-5 w-5 shrink-0 text-stone-400" />
                    <input
                        x-ref="input"
                        x-model="query"
                        @input="onInput()"
                ...
                <div id="command-palette-listbox" role="listbox" class="max-h-96 overflow-y-auto p-2">
                    <template x-if="query.trim().length === 0">
                        <p class="px-3 py-6 text-center text-sm text-stone-400 dark:text-stone-500">Aramaya başlamak için yazmaya başla.</p>
                    </template>
