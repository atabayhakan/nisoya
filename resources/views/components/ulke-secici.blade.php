...
@php
    $ulkeStili = \App\Support\Settings::get('gorunum.header_ulke_stili', 'bayrak_isim');
    $seciliBayrak = $country?->emoji ?: '🌍';
    $seciliAd = $country?->name_tr ?? $country?->name ?? 'Ülke';
@endphp
...
>
    ...
    <button
        ...
        aria-haspopup="true"
        aria-label="Ülke seç: {{ $seciliAd }}"
    >
        @if ($ulkeStili === 'kod_isim')
            <span class="rounded bg-stone-200/80 px-1.5 py-0.5 text-[10px] font-bold text-stone-700 dark:bg-stone-700 dark:text-stone-300">{{ $country?->code ?? '🌍' }}</span>
            <span class="hidden max-w-[130px] truncate sm:inline">{{ $seciliAd }}</span>
        @elseif ($ulkeStili === 'bayrak_yalniz')
            <span class="text-sm leading-none" role="img" aria-hidden="true">{{ $seciliBayrak }}</span>
            <span class="sr-only">{{ $seciliAd }}</span>
        @else
            {{-- Varsayılan: bayrak_isim --}}
            <span class="text-sm leading-none" role="img" aria-hidden="true">{{ $seciliBayrak }}</span>
            <span class="hidden max-w-[130px] truncate sm:inline font-medium">{{ $seciliAd }}</span>
        @endif
        <x-heroicon-o-chevron-down class="h-3 w-3 text-stone-400 transition-transform duration-200 dark:text-stone-500" ::class="acik ? 'rotate-180' : ''" />
    </button>
    ...
    <div
        ...
        class="hidden sm:block absolute right-0 top-full mt-2 w-80 overflow-hidden rounded-2xl border border-stone-200/90 bg-white/95 p-3 shadow-xl backdrop-blur-md ring-1 ring-black/5 dark:border-stone-800 dark:bg-stone-900/95 z-50"
        x-cloak
    >
        <div class="mb-2 flex items-center justify-between px-1">
            <span class="text-xs font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">Ülke Seçimi</span>
            <span class="rounded-full bg-stone-100 px-2 py-0.5 text-[10px] font-bold text-stone-600 dark:bg-stone-800 dark:text-stone-400">{{ count($countries) }} Ülke</span>
        </div>

        {{-- Arama Çubuğu --}}
        <div class="relative mb-2">
            <x-heroicon-o-magnifying-glass class="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-stone-400 dark:text-stone-500" />
            <input
                type="text"
                x-ref="aramaGirdisi"
                x-model="arama"
                ...
                <li x-show="!arama || '{{ Str::lower($ulke->name_tr) }} {{ Str::lower($ulke->code) }}'.includes(arama.toLowerCase())">
                    ...
                    >
                        <div class="flex items-center gap-2.5 min-w-0">
                            <span class="text-base leading-none" role="img" aria-hidden="true">{{ $ulke->emoji ?? '🌍' }}</span>
                            <span class="truncate">{{ $ulke->name_tr }}</span>
                            <span class="rounded bg-stone-100 px-1 py-0.5 text-[9px] font-bold text-stone-500 dark:bg-stone-800 dark:text-stone-400">{{ $ulke->code }}</span>
                        </div>
                        ...
                        <li>
                            ...
                            >
                                <div class="flex items-center gap-3 min-w-0">
                                    <span class="text-xl leading-none" role="img" aria-hidden="true">{{ $ulke->emoji ?? '🌍' }}</span>
                                    <span class="truncate">{{ $ulke->name_tr }}</span>
                                    <span class="rounded bg-stone-100 px-1.5 py-0.5 text-[10px] font-bold text-stone-500 dark:bg-stone-800 dark:text-stone-400">{{ $ulke->code }}</span>
                                </div>
                                @if ($country?->code === $ulke->code)
