...
{{-- ARAMA YORUMU ŞERİDİ — tek kaynak (klasik + vitrin).
     ...
     Gelen liste ZATEN insan diline çevrilmiş oluyor
     (DogalDilArama::etiketler). Canlıda önce ham değerler basılıyordu
     ("Kategori: ev-temizligi · Ülke: DE"); kullanıcıya URL slug'ı ve ülke
     kodu göstermek, "anladım" demenin amacını boşa çıkarıyordu.
...
@if (! empty($yorum))
    <div {{ $attributes->merge(['class' => 'rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 dark:border-stone-700 dark:bg-stone-900/40']) }}>
        <div class="flex flex-wrap items-center gap-x-2 gap-y-1.5 text-sm">
            ...
            @foreach ($yorum as $parca)
                <span class="rounded-full bg-white px-2.5 py-1 text-xs font-medium text-stone-700 ring-1 ring-stone-200 dark:bg-stone-800 dark:text-stone-200 dark:ring-stone-700">
                    {{ $parca['etiket'] }}: {{ $parca['deger'] }}
                </span>
            @endforeach
