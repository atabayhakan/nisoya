<x-layouts.guest title="Hesap Kurtarma — Nisoya">
    <h1 class="text-xl font-bold text-stone-900 dark:text-stone-50">Hesabını kurtar</h1>
    ...
    <form method="POST" action="{{ route('account-recovery.store') }}" class="mt-6 space-y-4">
        ...
        <div>
            <label for="code" class="block text-sm font-medium text-stone-700 dark:text-stone-300">Kurtarma kodu</label>
            <input id="code" name="code" type="text" value="{{ old('code') }}" required autocomplete="one-time-code"
                   ...
                   class="mt-1 w-full rounded-lg border-stone-300 px-3 py-2 font-mono uppercase tracking-wider shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100 dark:placeholder-stone-500">
            @error('code') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            <p class="mt-1 text-xs text-stone-600 dark:text-stone-400">Her kod yalnızca bir kez kullanılabilir.</p>
        </div>

        <div>
            <label for="password" class="block text-sm font-medium text-stone-700 dark:text-stone-300">Yeni parola</label>
        ...
        <div>
            <label for="password_confirmation" class="block text-sm font-medium text-stone-700 dark:text-stone-300">Yeni parola (tekrar)</label>
    ...
    <p class="mt-4 rounded-lg bg-stone-50 px-4 py-3 text-xs text-stone-500 dark:bg-stone-800 dark:text-stone-400">
