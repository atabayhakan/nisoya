<x-layouts.app title="İletişim — Nisoya" description="Sorun, önerin veya bir şikayetin mi var? Nisoya ekibine buradan ulaş.">
    <div class="mx-auto max-w-5xl px-4 py-12">
        ...
        <p class="mt-2 text-stone-600">Soru, öneri veya bir sorun mu var? Aşağıdaki formu doldur, sana en kısa sürede dönüş yapalım.</p>
        ...
        <div class="mt-10 grid gap-8 lg:grid-cols-5">
            ...
            <div class="lg:col-span-3">
                <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
                    @if (session('status'))
                        <div class="mb-6 flex items-start gap-2 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
                    ...
                    <form method="POST" action="{{ route('contact.store') }}" class="space-y-4">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label for="phone" class="block text-sm font-medium text-stone-700">Telefon <span class="text-stone-600">(ops.)</span></label>
                                <input id="phone" name="phone" type="tel" maxlength="50" value="{{ old('phone') }}"
                                       class="mt-1 w-full rounded-lg border-stone-300 px-3 py-2 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                                @error('phone') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="category" class="block text-sm font-medium text-stone-700">Konu</label>
                        ...
                        <div>
                            <label for="message" class="block text-sm font-medium text-stone-700">Mesajın</label>
                            <textarea id="message" name="message" rows="6" required minlength="10" maxlength="5000"
                                      placeholder="Nasıl yardımcı olabiliriz?"
                                      class="mt-1 w-full rounded-lg border-stone-300 px-3 py-2 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('message') }}</textarea>
                            @error('message') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>

                        <button type="submit" class="w-full rounded-lg bg-emerald-700 px-5 py-2.5 font-semibold text-white transition hover:bg-emerald-800 sm:w-auto">
                            Mesajı Gönder
                        </button>
                    </form>
                </div>
            ...
            <div class="space-y-4 lg:col-span-2">
                <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                    ...
                    <a href="mailto:{{ setting('iletisim.eposta') }}" class="mt-1 block text-sm text-emerald-700 hover:underline">{{ setting('iletisim.eposta') }}</a>
                </div>

                @if (setting('iletisim.telefon'))
                    <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                        <div class="text-2xl">📞</div>
                        <h2 class="mt-2 font-semibold text-stone-800">Telefon</h2>
                        <a href="tel:{{ setting('iletisim.telefon') }}" class="mt-1 block text-sm text-emerald-700 hover:underline">{{ setting('iletisim.telefon') }}</a>
                    </div>
                @endif
                ...
                @if (setting('iletisim.adres'))
                    <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                        <div class="text-2xl">📍</div>
                        <h2 class="mt-2 font-semibold text-stone-800">Adres</h2>
                        <p class="mt-1 text-sm text-stone-600">{{ setting('iletisim.adres') }}</p>
                    </div>
                @endif
                ...
                @if (setting('iletisim.calisma_saatleri'))
                    <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                        <div class="text-2xl">🕐</div>
                        <h2 class="mt-2 font-semibold text-stone-800">Yanıt süresi</h2>
                        <p class="mt-1 text-sm text-stone-600">{{ setting('iletisim.calisma_saatleri') }}</p>
                    </div>
                @endif
                ...
                @if (setting('footer.sosyal_instagram') || setting('footer.sosyal_facebook') || setting('footer.sosyal_whatsapp'))
                    <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                        <h2 class="font-semibold text-stone-800">Sosyal medya</h2>
