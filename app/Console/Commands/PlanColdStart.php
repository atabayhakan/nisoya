...
class PlanColdStart extends Command
{
    ...
    public function handle(CountryLiquidity $liquidity): int
    {
        ...
        $created = 0;
        $open = DB::table('global_growth_tasks')->whereIn('status', ['suggested', 'in_progress'])
            ->get(['country_code', 'action_key'])->mapWithKeys(fn ($task) => [$task->country_code.':'.$task->action_key => true]);
        foreach ($liquidity->snapshot(GeoContext::global())['rows'] as $row) {
            if ($row['stage'] !== 'cold_start') {
                continue;
            }
            $action = $row['accounts'] === 0 ? 'discover_community' : 'draft_local_campaign';
            if ($open->has($row['code'].':'.$action)) {
                continue;
            }
