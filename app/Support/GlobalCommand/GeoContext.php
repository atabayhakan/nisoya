...
final readonly class GeoContext
{
    ...
    public static function fromSelection(array $input): self
    {
        ...
        return new self('city', countryCode: $country->code, cityId: (int) $city->id,
            ...
            cityKeys: app(CityNames::class)->keys($city));
    }
    ...
    {
        return hash('sha256', json_encode([$this->selection(), $this->cityName, $this->cityKeys], JSON_THROW_ON_ERROR));
    }
    ...
    public function apply(Builder $query): Builder
    {
        ...
        if ($this->mode === 'city') {
            $query->whereIn($query->getModel()->qualifyColumn('city_key'), $this->cityKeys);
        }
