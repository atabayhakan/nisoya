...
class SyncDiasporaAccount implements ShouldBeUnique, ShouldQueue
{
    ...
    // Rate-limit releases consume attempts; only actual exceptions exhaust retries.
    public int $tries = 0;
    ...
    public function handle(StrictDiasporaSync $sync): void
    {
        ...
        if (! $cache instanceof LockProvider) {
            throw new \RuntimeException('Yapılandırılmış önbellek deposu (global-command.diaspora_sync_cache_store) atomik kilit desteklemiyor.');
        }
