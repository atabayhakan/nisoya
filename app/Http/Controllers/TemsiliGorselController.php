...
/**
 * İlana (herhangi bir tipte) temsilî kapak görseli üretir — panelden elle
 * tetiklenen yol. Aynı üretim otomatik de tetiklenebiliyor, bkz.
 * App\Console\Commands\TemsiliGorselOtomatikUret.
 *
 ...
 *
 * Kapılar üst üste: yetki (kendi ilanı) → uygunluk (görseli yok + özellik
 * açık) → hız sınırı (üretim para harcar). Uygunluk kontrolü servisin
 * içinde, çünkü aynı kontrol düğmeyi göstermek için de kullanılıyor ve iki
 * yere yazılırsa biri güncellenip diğeri unutulur.
 */
class TemsiliGorselController extends Controller
{
    ...
    public function store(Listing $listing): RedirectResponse
    {
        ...
        if (! $this->uretici->uygunMu($listing)) {
            ...
            // bir şey bekliyordur.
            return $geri->with('status', 'Bu ilana temsilî görsel eklenemez — yalnız görseli olmayan ilanlar için.');
        }
